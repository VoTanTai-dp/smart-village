...
const Dashboard = () => {
        ...
        const initSensorWs = () => {
            ...
            sensorWs.onmessage = (event) => {
                try {
                    ...
                    if (msg.type === 'sensor') {
                        ...
                        setCamerasData((prev) => {
                            ...
                            cam.latestRecord = {
                                ...
                                temperature,
                                humidity,
                                timestamp: timestamp || latest.timestamp || null,
                            };
                    ...
                    if (msg.type === 'count') {
                        const { cameraId, sessionId, people, vehicle, atTime } = msg;
                        if (!cameraId) return;
                        ...
                        setCamerasData((prev) => {
                            ...
                            const cam = { ...list[idx] };
                            const latest = cam.latestRecord || {};
                            const ts = timestamp || latest.timestamp || null;
    ...
    const addressKeys = Object.keys(groupedCameras).sort((a, b) => a.localeCompare(b));

    const activeCameras =
        activeAddress === 'ALL' ? camerasData : groupedCameras[activeAddress] || [];

    const formatTemp = (value) =>
        value === null || value === undefined ? 'N/A' : `${Number(value).toFixed(1)}°C`;
        ...
        <div className="dashboard-container">
            <div className="container">
                    ...
                    <div className="nav-links">
                        ...
                        {addressKeys.map((addr) => (
                            <a
                                key={addr}
                                href={`#${addr}`}
                                className={activeAddress === addr ? 'active' : ''}
                                onClick={(e) => {
                                    e.preventDefault();
                ...
                <div className="d-flex flex-column">
                    {activeAddress !== 'ALL' && (
                        <h2 className="area-title mb-3">Area: {activeAddress}</h2>
                    )}
                    ...
                    {activeCameras.map((cam) => {
                        ...
                        return (
                            <div className="sensor-card fade-in" key={cam.cameraId}>
                                <div className="card-header-custom">
                                    <h3 className="camera-name">
                                        Camera CAM-{cam.cameraId.toString().padStart(3, '0')}
                                    </h3>
                                    <div className="row camera-info gy-2">
                                        <div className="col-6 col-md-3">
                                            <strong>IP:</strong> {cam.ip || 'N/A'}
                                        </div>
                                        <div className="col-6 col-md-3">
                                            <strong>Port:</strong> {cam.port || 'N/A'}
                                        </div>
                                        <div className="col-12 col-md-6">
                                            <strong>Address:</strong> {cam.address || 'N/A'}
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="col-md-4">
                                        ...
                                        <div className="latest-record-box">
                                            <div className="record-item">
                                                ...
                                                <span className="value">{formatTemp(latest.temperature)}</span>
                                            </div>
                                            <div className="record-item">
                                                <span className="label">Humidity</span>
                                                <span className="value">{formatHumidity(latest.humidity)}</span>
                                            </div>
                                            <div className="record-item">
                                                <span className="label">People</span>
                                                <span className="value">{formatCount(latest.people)}</span>
                                            </div>
                                            <div className="record-item">
                                                <span className="label">Vehicle</span>
                                                <span className="value">{formatCount(latest.vehicle)}</span>
                                            </div>
                                        ...
                                        <div className="table-responsive historical-table-wrapper">
                                            <table className="table custom-table">
                                                ...
                                                <tbody>
                                                    {history.length === 0 && (
                                                        <tr>
                                                            <td colSpan={5}>No historical data.</td>
                                                        </tr>
                                                    )}
                                                    {history.map((row, idx) => (
                                                        <tr key={idx}>
                    ...
                    {activeCameras.length === 0 && (
                        <div className="text-center py-5 text-muted">No cameras found.</div>
                    )}
                </div>
            </div>
        </div>
